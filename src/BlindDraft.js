...
class MyDraft {
    ...
    addTranslation(chunkNumber, translation) {
        if (chunkNumber >= 0 && chunkNumber < this.chunks.length) {
            this.chunks[chunkNumber].translation = translation;
        }
    }

    getTranslation() {
        return this.chunks.map( x => x.translation);
    }

    translationComplete() {
        let values = this.chunks.map(x => x.translation != "" );
        return values.every(complete => complete == true);
    }

    getNextChunk(num) {
        let clamped = Math.max(Math.min(num, this.chunks.length-1), 0);
        return this.chunks[clamped];
    }
}

const myDraft = new MyDraft();
...
function BlindDraft() {
    ...
    return (
        <div>
            <div>{chunk.text}</div>
            <textarea id="draftText">Write draft...</textarea>
            <button onClick={() => {
                myDraft.addTranslation(chunkNumber, document.getElementById('draftText').value);
                if (myDraft.translationComplete()) {
                    alert(myDraft.getTranslation());
                }
